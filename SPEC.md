...
3. **Guardrail the known weakness.** Every specific authority a lens cites (statute, regulation, case, FPPC opinion) is extracted deterministically (regex, not a model) and flagged **UNVERIFIED — check before relying**. The tool never asserts black-letter law as settled. The failure mode we defend against is confident mischaracterization on top of plausible-looking cites. A *verifier adapter* can upgrade specific flags. Two exist: a manual corpus check (Round 1: FPPC-adjacent authorities against the real FPPC opinion database) and, since Round 3, an automated stage (`python3 -m lenses verify <qid>`) that fans out search-enabled worker calls per flagged authority — existence, citation form, and characterization only; explicitly not a good-law/Shepardizing check.
